...
Na máquina VPN mude para a directoria `/etc/openvpn/`, corra `cp -r /usr/share/easy-rsa /etc/openvpn/` e `cp vars.example /etc/openvpn/easy-rsa/vars`.
Edite o ficheiro `vars`.
Altere as últimas linhas.
...
5. Copie os seguintes ficheiros que foram gerados no servidor VPN (estão na pasta `keys`) para a área do *root* do PC1: `ca.crt`, `client1.crt` e `client1.key`.
Os ficheiros do cliente (principalmente as chaves) devem ser apagadas do servidor e mantidas secretas.
Sugestão: use o comando `scp`.
Para tal defina a password de *root* no PC1.
...
7. No servidor de VPN, copie os ficheiros gerados (`ca.crt` `dh1024.pem` `server.key` `server.crt`) para a pasta `/etc/openvpn/`.
...
11. Observe as mensagens no cliente e servidor.
Verifique que já estão ligados.

12. Verifique as rotas presentes no router 1.
...
13. Coloque o cliente em background com *\^z* (Ctrl + z) seguido do `bg`.
Tente agora aceder do PC1 à rede que está bloqueada: `192.168.0.0/24`.

14. Faça `traceroute` do PC1 para o servidor 4.
